...
window.addEventListener('DOMContentLoaded', () => {
  ...
  if (adminLogoutBtn) adminLogoutBtn.addEventListener('click', () => auth.signOut());
...
function updateCategoryCounts() {
  ...
  const countFF = document.getElementById('count-freefire');
  const countYT = document.getElementById('count-youtube');
  const countIG = document.getElementById('count-instagram');

  if (countAll) countAll.textContent = `${products.length} Items`;
  if (countFF) countFF.textContent = `${products.filter(p => p.category === 'freefire').length} Items`;
  if (countYT) countYT.textContent = `${products.filter(p => p.category === 'youtube').length} Items`;
  if (countIG) countIG.textContent = `${products.filter(p => p.category === 'instagram').length} Items`;
}
